(()=>{
  const {D,clusterColor,fmtMoney,fmtNumber}=window.HE;
  const entityColours=['#397fd8','#8d67ce','#22a99a'];
  ...
  const defaults=state.mode==='country'?['FR','DE','NL']:[nzOrganisations[0]?.key,nzOrganisations[1]?.key].filter(Boolean);state.items=(requested.length?requested:defaults).filter(value=>options().some(option=>option.value===value));
