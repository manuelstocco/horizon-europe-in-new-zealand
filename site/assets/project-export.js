(() => {
  ...
  function pptProject(pptx,slide,item,plan,page){
    ...
    pptText(slide,`${clusterName(p.clusterCode)}  ·  ${schemeDisplayName(p.schemeCode)}  ·  ${p.status||'Status not reported'}`,.55,1.7,12.2,.22,{fontSize:12.3,bold:true,color:accent,fit:'shrink'});
  ...
  async function createPdfBytes(plan){
    ...
    plan.slides.forEach((item,index)=>{
      ...
      if(item.type==='summary'){
        ...
        const d=item.data,p=d.project,accent=hex(HE.clusterColor(p.clusterCode)),nzAll=d.nz,coordinatorNz=d.coordinator?.countryCode==='NZ',footprintInRight=!d.preview.length&&nzAll.length<=2;
        header(page,p.acronym,truncate(normaliseProjectTitle(p.title,p.acronym),118),index+1,accent);fit(page,`${clusterName(p.clusterCode)} · ${schemeDisplayName(p.schemeCode)} · ${p.status||'Status not reported'}`,40,125,10,accent,bold,880,7.8);
